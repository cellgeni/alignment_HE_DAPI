...
def OptFlowRegistration(img1, img2, n_pyr = 5, n_iter = 4, tile_size = 1000):
    ofreg = OptFlowRegistrator()
    ofreg.ref_img = img1
    ofreg.mov_img = img2
    ofreg.num_pyr_lvl = n_pyr
    ofreg.num_iterations = n_iter
    ofreg.tile_size = tile_size
    #ofreg.use_full_res_img = True
    ofreg.use_dog = True
...
def warp_opt_flow(img, flow_map):
    warper = Warper()
    warper.image = img
    warper.flow = flow_map
    img2_optflow_reg = warper.warp()
    return img2_optflow_reg

def warp_HE_image(img, trans_mat, flow_map):
    img2_reg_3ch = np.zeros_like(img)
    for i in range(img.shape[2]):
        img2_reg_3ch[:,:,i] = transform_img_with_tmat(img[:,:,i], img[:,:,i].shape, trans_mat)
        img2_reg_3ch[:,:,i] = warp_opt_flow(img2_reg_3ch[:,:,i], flow_map)
    return img2_reg_3ch
...
def RegisterOneImage(img_HE_path, img_X_path, rot_clock, rot_anticlock, flip_x, flip_y, n_pyr, n_iter, max_int_clip, n_pyr_of, n_iter_of):
    ...
    transformation_matrix = FeatRegistration(img1, img2, n_pyr, n_iter)
    
    img2_reg = np.zeros_like(img2)
    img2_reg = transform_img_with_tmat(img2, img2.shape, transformation_matrix)
    
    del img_HE, img_X, img2
    gc.collect()
    
    print('optical flow registration')
    flow_map = OptFlowRegistration(img1, img2_reg, n_pyr_of, n_iter_of)
    
    print('warping HE image')
    img2_reg = warp_HE_image(img2_orig, transformation_matrix, flow_map)
    
    return img1_orig, img2_reg, transformation_matrix, dim_order, pixel_size, channel_names, flow_map
...
def prepare_paths(out_folder, name):
    ...
    path_flow_map = os.path.join(out_folder,name + '_flow_map.txt')
    return path_HE, path_DAPI, path_trmat, path_flow_map


def main(csv_table_path, out_folder, n_pyr = 5, n_iter = 3, max_int_clip = 50, n_pyr_of = 5, n_iter_of = 4):
    ...
    for i in range(table.shape[0]):
        print(table['Name'][i])
        img1_orig, img2_reg, transformation_matrix, dim_order, pixel_size, channel_names, flow_map = RegisterOneImage(table['HE_image_path'][i], table['DAPI_image_path'][i], bool(table['rotate_clockwise'][i]), bool(table['rotate_anticlockwise'][i]), bool(table['flip_x'][i]), bool(table['flip_y'][i]), n_pyr, n_iter, max_int_clip, n_pyr_of, n_iter_of)
        print('saving images')
        
        path_HE, path_DAPI, path_trmat, path_flow_map =  prepare_paths(out_folder, table['Name'][i])
        np.savetxt(path_trmat, transformation_matrix)
        np.savetxt(path_flow_map, flow_map)
